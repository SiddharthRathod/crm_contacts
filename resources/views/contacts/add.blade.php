        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body">                        
                        <form method="POST" action="javascript:void(0)" enctype="multipart/form-data" id="contactForm">
                            @csrf
                            ...
                            <div class="row mb-3 align-items-center">
                                ...
                                <div class="col-md-6">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="gender" id="gender_male" value="male" {{ old('gender') == 'male' ? 'checked':'' }} aria-describedby="gender-error">
                                            ...
                                            @if($def->field_type === 'select')
                                                ...
                                                <select name="custom_fields[{{ $def->id }}]" class="form-select @error('custom_fields.'. $def->id) is-invalid @enderror" @if($def->is_required) required @endif>
                                                    ...
                                                    @foreach($options as $opt)
                                                        <option value="{{ trim($opt) }}" {{ old('custom_fields.'.$def->id) == trim($opt) ? 'selected': ''}}>{{ trim($opt) }}</option>
                                                    @endforeach
                                                </select>
                                            @elseif($def->field_type === 'date')
                                                <input type="date" name="custom_fields[{{ $def->id }}]" value="{{ old('custom_fields.'.$def->id) }}" class="form-control @error('custom_fields.'. $def->id) is-invalid @enderror" @if($def->is_required) required @endif>
                                            @else
                                                <input type="{{ $def->field_type }}" name="custom_fields[{{ $def->id }}]" value="{{ old('custom_fields.'.$def->id) }}" class="form-control @error('custom_fields.'. $def->id) is-invalid @enderror" @if($def->is_required) required @endif>
                                            @endif
                ...
                $("#contactForm").validate({
                    ...
                    rules: {
                        ...
                        email: {
                            required: true,
                            email: true,
                            remote: {url: "{{ route('check-contact-email') }}",type: "post",data:{'_token': '{{ csrf_token() }}' }},
                        },
                        phone:"required",
                        gender:"required",
                    },
                    ...
                    submitHandler: function(form) {
                        ...
                        if(response.success){
                            ...
                            let errMsg = response.message || 'Failed to add contact';
